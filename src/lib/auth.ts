...
export const updateFileAsset = async (id: number, updateFile: UploadFile & { uploader_id?: bigint }): Promise<FileAsset | null> => {
  const client = await pool.connect();
  console.log("Updating file asset:", updateFile);

  try {
    const checkFile = await client.query(`SELECT id FROM file_assets WHERE id = $1 AND ($2::bigint IS NULL OR uploader_id = $2::bigint)`, [
      id,
      updateFile.uploader_id ? updateFile.uploader_id.toString() : null,
    ]);
    ...
    const uploaderId = updateFile.uploader_id ? updateFile.uploader_id.toString() : null;
